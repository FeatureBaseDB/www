...
So instead of 10s of millions of boolean attributes, we have just a few
...
Pilosa was designed for.
 
We stood up a 3 node Pilosa cluster on AWS c4.8xlarge instances, with one more
c4.8xlarge to load the data. We used our open source `pdk` tool to load the data
into Pilosa with the following arguments:
...
If we were to add our results to Mark's table, it would look as follows:
...
data, it already has this result precomputed, so the query time is mostly
network latency.
...
on Query 3 was particularly startling - performance along the lines of 8 Nvidia
Pascal Titan Xs is shocking. It looks like kdb+/q is beating us pretty soundly,
...
Pilosa for uses outside of it's original intent. We know that Pilosa's internal
bitmap compression format is not optimized for dense data, and more research has
been done in this area with exciting results (e.g. [roaring-run](https://arxiv.org/pdf/1603.06549.pdf)), so we have reason to believe that there is significant room for improvement in these numbers.
